...
class pDraw
{
    ...
    function drawStackedAreaChart($Format = NULL)
    {
        ...
        foreach ($SerieOrder as $Key => $SerieName) {
            ...
            if ($Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"]) {
                ...
                elseif ($LineR != VOID)
                    $LineColor = array("R" => $LineR, "G" => $LineG, "B" => $LineB, "Alpha" => $LineAlpha); else
                    $LineColor = $Color;
    ...
    function drawShape($X, $Y, $Shape, $PlotSize, $PlotBorder, $BorderSize, $R, $G, $B, $Alpha, $BorderR, $BorderG, $BorderB, $BorderAlpha)
    {
        if ($Shape == SERIE_SHAPE_FILLEDCIRCLE) {
            ...
            $this->drawRectangle($X - $PlotSize, $Y - $PlotSize, $X + $PlotSize, $Y + $PlotSize, array("R" => $R, "G" => $G, "B" => $B, "Alpha" => $Alpha)); elseif ($Shape == SERIE_SHAPE_CIRCLE)
            $this->drawCircle($X, $Y, $PlotSize, $PlotSize, array("R" => $R, "G" => $G, "B" => $B, "Alpha" => $Alpha)); elseif ($Shape == SERIE_SHAPE_DIAMOND) {
            $Pos = "";
            $Pos[] = $X - $PlotSize;
            $Pos[] = $Y;
            $Pos[] = $X;
